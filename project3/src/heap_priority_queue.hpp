...
template<class T>
HeapPriorityQueue<T>::HeapPriorityQueue(const HeapPriorityQueue<T>& to_copy) : gt(to_copy.gt), length(to_copy.length), used(to_copy.used)
{     pq = new T[length];
...
void HeapPriorityQueue<T>::percolate_down(int i){
	...
	while (in_heap(left_child(i)) ){
		...
		if (gt (pq[index] ,  pq[i]) /*or gt (pq[i] ,  pq[index]) or */) {
...
bool HeapPriorityQueue<T>::operator == (const PriorityQueue<T>& rhs) const {
	 ...
	 if (this == &rhs)
	  ...
	  ics::Iterator<T>& itself_i = this->abegin();
	  for (int i=0; i<used; ++i, ++rhs_i, ++itself_i){
	    if (*itself_i != *rhs_i)
	      return false;
	  }
	  return true;
...
T HeapPriorityQueue<T>::Iterator::erase() {
  ...
  int index_to_erase = -1;  // find the index the thing you want to erase is at.
 	  for (int i = 0; i < ref_pq->used; i++){
 		  if (ref_pq->pq[i] == this->it.peek()){
 			  index_to_erase = i;
 		  }
 	  }
  can_erase = false;
  T to_return = ref_pq->pq[index_to_erase];

  if(!ref_pq->in_heap(ref_pq->left_child(index_to_erase))){
	  ref_pq->pq[index_to_erase] = ref_pq->pq[ref_pq->used-1];
	  ref_pq->used--;
	  ref_pq->percolate_up(index_to_erase);
  }
  else{
	  ref_pq->percolate_down(index_to_erase);
  }
  ref_pq->mod_count++;
