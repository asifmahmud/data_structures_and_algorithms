...
//	std::cout << "k[i] is: " << k[i] << std::endl;
//	std::cout << "i is: " << i <<" m[k[i]] is: " << m[k[i]] << " values[i] is: " << values[i] << std::endl;
    if (m[k[i]] != values[i]){
...
TEST_F(MapTest, insert_iterator_param) {
  ...
  ASSERT_EQ(10, m.put(m1.abegin(),m1.aend()));
  ASSERT_FALSE(m.empty());
  ASSERT_EQ(10, m.size());
  ASSERT_EQ(m,m1);
}
...
TEST_F(MapTest, iterator_simple) {
  ...
  q.clear();
  for (ics::Iterator<EntryType>& it = m.abegin(); it != m.aend(); ++it)
    q.enqueue(*it);
  i = 0;
  for (EntryType x : q) {
    ASSERT_EQ(qkeys[i],x.first);
...
TEST_F(MapTest, iterator_erase) {
  ...
  MapType m;
  load(m,"fcijbdegah", new int[10]{6,3,9,10,2,4,5,7,1,8});
  ics::Iterator<EntryType>& it = m.abegin();
  ...
  MapType m2;
  ASSERT_EQ(6,m2.put(erased.abegin(),erased.aend()));
  ASSERT_EQ(6,m2.size());
  for (EntryType x : m2)
