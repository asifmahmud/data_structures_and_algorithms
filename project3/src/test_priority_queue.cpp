...
TEST_F(PriorityQueueTest, operator_stream_insert) {// <<
 ...
 PriorityQueueType q(gt_string);
 value << q;
 ASSERT_EQ("priority_queue[]", value.str());
...
TEST_F(PriorityQueueTest, iterator_erase) {
 ...
 PriorityQueueType q(gt_string);
 load(q,"fcijbdegah");
 ics::Iterator<std::string>& it = q.abegin();
...
TEST_F(PriorityQueueTest, iterator_erase_heap_special_case) {
 PriorityQueueType q(gt_string);
 load(q,"adbefc");
 for (ics::Iterator<std::string>& i = q.abegin(); i != q.aend(); ++i)
   if (*i == "f") {
     i.erase();
     break;
   }
...
TEST_F(PriorityQueueTest, constructors) {
 ...
 load(q,"fcijbdegah");
 PriorityQueueType q3(q.abegin(),q.aend(),lt_string);   //Ordered oppositely
 ASSERT_TRUE(unload(q, "abcdefghij"));
 ASSERT_NE(q,q3);
