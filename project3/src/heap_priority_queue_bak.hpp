...
HeapPriorityQueue<T>::HeapPriorityQueue(const HeapPriorityQueue<T>& to_copy) : gt(to_copy.gt), pq(new T[to_copy.size()])
{
	...
	for (auto i = 0; i < to_copy.used; ++i) {
		//this->pq[i] = to_copy.pq[i];
		this->enqueue(to_copy.pq[i]);
...
//	pq[i] = pq[used-1]; // place last value at the top
//	int index;
//	while (in_heap(left_child(i)) || in_heap(right_child(i))) {
...
//	}
	pq[i] = pq[used-1];
	int index;
	if (!in_heap(left_child(i))){ // last node
		std::cout << "here 1" << std::endl;
		return;
	}
	else if (gt(pq[i], pq[left_child(i)]) && !in_heap(right_child(i))){ // if node is greater priority than left and right, or if it is at the bottom
		...
		index = (gt(pq[left_child(i)], pq[right_child(i)]) ? left_child(i) : right_child(i));
		std::cout << "gt = " << gt (pq[i] ,  pq[index])  << "i = " << pq[i]<< " index = " << pq[index]<< std::endl;
