...
const std::string EquivalenceClassError::what () const {
  return "HashEquivalenceClassError " + message;
};
...
template <class T>
HashEquivalence<T>::HashEquivalence(int (*ahash)(const T& k)) : hash(ahash){
	ics::HashMap<T,T>   p(hash);
	ics::HashMap<T,int> r(hash);
	parent = p;
	root_size = r;
}
...
template<class T>
void HashEquivalence<T>::add_singleton (T a) {
  if (parent.has_key(a)) {
...
template<class T>
bool HashEquivalence<T>::in_same_class (T a, T b) {
  if (!parent.has_key(a) or !parent.has_key(b)) {
	std::ostringstream exc;
	exc << "HashEquivalence.compress_to_root: a(" << (!parent.has_key(a) ? a : b) << ") not in an equivalence class";
	throw EquivalenceClassError(exc.str());
  }
...
//  is not already a value in the Equivalence (were never added as singletons)
template<class T>
void HashEquivalence<T>::merge_classes_of (T a, T b) {
  if (!parent.has_key(a) or !parent.has_key(b)) {
	std::ostringstream exc;
	exc << "HashEquivalence.compress_to_root: a(" << (!parent.has_key(a) ? a : b) << ") not in an equivalence class";
	throw EquivalenceClassError(exc.str());
...
ics::ArraySet<ics::ArraySet<T>> HashEquivalence<T>::classes () {
	...
	for (auto k : parent){compress_to_root(k.first);};
