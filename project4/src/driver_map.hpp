...
class DriverMap {
  ...
  private:
    ...
    std::string menu_prompt (std::string preface) {
      ...
      std::cout << preface+"  [  - put:[]      g  - get:[]            l  - load from file" << std::endl;
      std::cout << preface+"  p  - put         m  - empty             it - iterator commands" << std::endl;
      std::cout << preface+"  P  - put         s  - size              q  - quit" << std::endl;
      std::cout << preface+"  e  - erase       k  - has_key"        << std::endl;
      std::cout << preface+"  x  - clear       v  - has_value"      << std::endl;
      std::cout << preface+"  =  - =           <  - <<"             << std::endl;
      std::cout << preface+"                   r  - relations"      << std::endl;

      std::string allowable[] = {"[","p","P","e","x","=","g","m","s","k","v","<","r","l","it","q",""};
  ...
  void process_iterator_commands(MapType& m, std::string preface) {
    ...
    ics::Iterator<MapEntry>& i = m.abegin();
    for (;;)
      try {
  ...
  void process_commands(std::string preface) {
    for (;;) try {
      ...
      if (command == "[") {
        ...
        std::cout << preface+"  put = " << m.put(m2.abegin(),m2.aend()) << std::endl;
      }
        ...
        std::cout << preface+"  get = " << ((const MapType)m)[k] << std::endl;
      }
