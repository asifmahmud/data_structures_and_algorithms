...
//template<class T>
//HeapPriorityQueue<T>::HeapPriorityQueue(std::initializer_list<T> i, bool (*agt)(const T& a, const T& b))
//  : gt(agt) {
//  pq = new T[length];
...
bool HeapPriorityQueue<T>::operator == (const PriorityQueue<T>& rhs) const {
  ...
  ics::Iterator<T> &l = this->abegin(), &r = rhs.abegin();
...
std::ostream& operator << (std::ostream& outs, const HeapPriorityQueue<T>& p) {
  if (p.empty()) {
    ...
    outs << "priority_queue[";
    ArrayStack<T> temp(p.abegin(), p.aend());
...
template<class T>
HeapPriorityQueue<T>::Iterator::Iterator(HeapPriorityQueue<T>* fof, bool begin) : it(nullptr), ref_pq(fof) {
  if (begin)
    it = *fof;
...
template<class T>
HeapPriorityQueue<T>::Iterator::~Iterator() {}//it.~HeapPriorityQueue() called implicitly.
