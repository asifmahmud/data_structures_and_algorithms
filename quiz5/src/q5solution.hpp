...
class NTN {
  public:
    ...
    NTN (const NTN<T>& n)
      : value(n.value), children(n.children.abegin(),n.children.aend())
    {}

    NTN (T value, ics::ArraySet<NTN> values)
      : value(value), children(values.abegin(),values.aend())
    {}
...
std::string longest_word (const DTN& root) {
	std::string max_word = root.word_to_here;
	for (auto i : root.children) {
		max_word = (longest_word(i.second).size() > max_word.size()? longest_word(i.second) : max_word);
	}
	return max_word;
