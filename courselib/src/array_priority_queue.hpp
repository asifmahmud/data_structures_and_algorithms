...
template<class T> class ArrayPriorityQueue : public PriorityQueue<T>  {
  public:
    ...
    //KLUDGE: not usable yet. ArrayPriorityQueue(std::initializer_list<T> il,bool (*agt)(const T& a, const T& b));
    ...
    class Iterator : public ics::Iterator<T> {
      public:
        ...
        Iterator(ArrayPriorityQueue<T>* fof, int initial);
        virtual ~Iterator();
        virtual T           erase();
        virtual std::string str  () const;
...
template<class T>
ArrayPriorityQueue<T>::ArrayPriorityQueue(bool (*agt)(const T& a, const T& b)) : gt(agt) {
  pq = new T[length];
}

template<class T>
ArrayPriorityQueue<T>::ArrayPriorityQueue(int initial_length, bool (*agt)(const T& a, const T& b))
  : gt(agt), length(initial_length) {
  if (length < 0)
    length = 0;
  pq = new T[length];
}
...
//template<class T>
//ArrayPriorityQueue<T>::ArrayPriorityQueue(std::initializer_list<T> i, bool (*agt)(const T& a, const T& b))
//  : gt(agt) {
//  pq = new T[length];
//  enqueue(il.abegin(),il.aend());
//}
...
int ArrayPriorityQueue<T>::enqueue(const T& element) {
  ...
  for (int i=used-2; i>=0; --i)
    if (gt(pq[i],pq[i+1])) {//KLUDGE swap<T>?
      T temp = pq[i];
      pq[i] = pq[i+1];
      pq[i+1] = temp;
    }else
...
//KLUDGE: memory-leak
template<class T>
auto ArrayPriorityQueue<T>::abegin () const -> ics::Iterator<T>& {
  return *(new Iterator(const_cast<ArrayPriorityQueue<T>*>(this),used-1));
}

//KLUDGE: memory-leak
template<class T>
auto ArrayPriorityQueue<T>::aend () const -> ics::Iterator<T>& {
...
template<class T>
ArrayPriorityQueue<T>::Iterator::Iterator(ArrayPriorityQueue<T>* fof, int initial) : current(initial), ref_pq(fof) {
