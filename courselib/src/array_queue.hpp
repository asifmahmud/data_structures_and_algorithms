...
template<class T> class ArrayQueue : public Queue<T>  {
  public:
    ...
    //KLUDGE: not usable yet. ArrayQueue(std::initializer_list<T> il);
    ...
    template<class T2>
    friend std::ostream& operator << (std::ostream& outs, const ArrayQueue<T2>& s);
    ...
    class Iterator : public ics::Iterator<T> {
      public:
        ...
        Iterator(ArrayQueue<T>* fof, int initial);
        virtual ~Iterator();
        virtual T           erase();
        virtual std::string str  () const;
...
//template<class T>
//ArrayQueue<T>::ArrayQueue(std::initializer_list<T> i) {
//  queue = new T[length];
...
template<class T>
std::ostream& operator << (std::ostream& outs, const ArrayQueue<T>& s) {
  if (s.empty()) {
    outs << "queue[]";
    return outs;
  }else{
    outs << "queue[" << s.queue[s.front];
    for (int i=(s.front+1)%s.length; i!=s.rear; i=(i+1)%s.length) {
      outs << ",";
      outs << s.queue[i];
    }
    outs <<"]:rear";
...
//KLUDGE: memory-leak
template<class T>
auto ArrayQueue<T>::abegin () const -> ics::Iterator<T>& {
  return *(new Iterator(const_cast<ArrayQueue<T>*>(this),front));
}

//KLUDGE: memory-leak
template<class T>
auto ArrayQueue<T>::aend () const -> ics::Iterator<T>& {
...
template<class T>
ArrayQueue<T>::Iterator::Iterator(ArrayQueue<T>* fof, int initial) : current(initial), ref_queue(fof) {
...
const ics::Iterator<T>& ArrayQueue<T>::Iterator::operator ++ () {
  ...
  if (can_erase && current != ref_queue->rear)
