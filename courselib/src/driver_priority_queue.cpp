...
class DriverPriorityQueue {
  public:
    ...
    DriverPriorityQueue() : q(NULL) {
        ...
        q = ArrayPriorityQueue<std::string>(gt);
        process_commands("");
      };
  ...
  private:
    ...
    PriorityQueueType& prompt_queue(std::string preface, std::string message = "  Enter element for q2") {
      static ics::ArrayPriorityQueue<std::string> q2([](const std::string& a, const std::string& b) {return a < b;});
      q2.clear();
      for (;;) {
    ...
    std::string menu_prompt (std::string preface) {
      ...
      std::cout << preface+"  e  - enqueue     m  - empty             l  - load from file" << std::endl;
      std::cout << preface+"  E  - enqueue     s  - size              it - iterator commands" << std::endl;
      std::cout << preface+"  d  - dequeue     p  - peek              q  - quit" << std::endl;
      std::cout << preface+"  x  - clear       <  - <<"        << std::endl;
      std::cout << preface+"  =  - =           r  - relations" << std::endl;

      std::string allowable[] = {"e","E","d","x","=","m","s","p","<","r","l","it","q",""};
    ...
    void process_iterator_commands(PriorityQueueType& q, std::string preface) {
      ...
      ics::Iterator<std::string>& i = q.abegin();
      for (;;)
        try {
    ...
    void process_commands(std::string preface) {
      for (;;) try {
        ...
        if (command == "e") {
          ...
          std::cout << "  dequeue = " << q.enqueue(q2.abegin(),q2.aend()) << std::endl;;
        }
          ...
          std::cout << preface+"  s == s = " << (q == q) << std::endl;
          std::cout << preface+"  s != s = " << (q != q) << std::endl;

          PriorityQueueType q2(prompt_queue(preface));
          std::cout << preface+"  s = " << q << " ?? q2 = " << q2 << std::endl;
          std::cout << preface+"  s == q2 = " << (q == q2) << std::endl;
          std::cout << preface+"  s != q2 = " << (q != q2) << std::endl;
        }

        else if (command == "l") {
          std::ifstream in_queue;
          ics::safe_open(in_queue,preface+"  Enter file name to read", "load.txt");
          std::string e;
