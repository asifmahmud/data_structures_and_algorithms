  ...
  public:
    ...
    class Iterator : public ics::Iterator<Entry> {
      public:
        ...
        Iterator(ArrayMap<KEY,T>* fof, int initial);
        virtual ~Iterator() {}
        virtual Entry       erase();
        virtual std::string str  () const;
    ...
    public://KLUDGE: private: friend not found
    Entry* map;
...
//template<class KEY,class T>
//ArrayMap<KEY,T>::ArrayMap(std::initializer_list<Entry> i) {
//  map = new T[length];
...
T ArrayMap<KEY,T>::put(const KEY& key, const T& value) {
  ...
  map[used++] = ics::make_pair(key,value);
  ++mod_count;
...
bool ArrayMap<KEY,T>::operator == (const Map<KEY,T>& rhs) const {
  ...
  if (used != rhs.size())
    return false;
  for (int i=0; i<used; ++i)
    if (map[i].second != rhs[map[i].first])
      return false;
...
std::ostream& operator << (std::ostream& outs, const ArrayMap<KEY,T>& m) {
  if (m.empty()) {
    ...
    for (int i=0; i<m.used; ++i) {
      outs << m.map[i].first << "->" << m.map[i].second; //KLUDGE
      outs << (i == m.used-1 ? "]" : ",");
    }
  }
  return outs;
...
//KLUDGE: memory-leak
template<class KEY,class T>
auto ArrayMap<KEY,T>::abegin () const -> ics::Iterator<Entry>& {
  return *(new Iterator(const_cast<ArrayMap<KEY,T>*>(this),0));
}

//KLUDGE: memory-leak
template<class KEY,class T>
auto ArrayMap<KEY,T>::aend () const -> ics::Iterator<Entry>& {
...
template<class KEY,class T>
ArrayMap<KEY,T>::Iterator::Iterator(ArrayMap<KEY,T>* fof, int initial) : current(initial), ref_map(fof) {
...
auto  ArrayMap<KEY,T>::Iterator::operator ++ () -> const ics::Iterator<ics::pair<KEY,T>>& {
  ...
  if (can_erase && current < ref_map->used)
    ++current;
  can_erase = true;
  return *this;
}
